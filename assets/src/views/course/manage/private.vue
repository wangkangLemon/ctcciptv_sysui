...
<template>
	<article class="block-manage">
		<section class="right-content" style="width: 100%;">
			...
			<div class="content-list">
				<div class="search">
					<section class="fi">
						<i>标题</i>
						<el-input id="input" v-model="section.course_name" placeholder="请输入标题" @keyup.enter.native="fetchCourseLists" auto-complete="off"></el-input>
					</section>
				...
				<el-table v-loading="section.loading" border :data="section.data">
					...
					<el-table-column prop="operate" label="操作" width="150" fixed="right">
						<template scope="scope">
							<el-button @click="edit(scope.row)" type="text" size="small">查看
							</el-button>
				...
				<div class="block">
					<el-pagination @size-change="handleSizeChange" @current-change="sectionPageChange" :current-page="section.page" :page-size="section.pagesize" :page-sizes="[15, 30, 60, 100]" layout="total,sizes,prev, pager,next,jumper" :total="section.total">
					</el-pagination>
				</div>
			</div>
		</section>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//                  }
			//                  else{
			//                      this.$router.push({ name:'course-manage-addCourse',params:{addcourseInfo:this.section,handle:'add'},query:{
			//                      	category_name:this.section.category_name,
