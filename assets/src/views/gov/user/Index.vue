...
<template>
	<article id="medical-index-container">
		...
		<el-dialog class="show-detail" title="查看管理员" v-model="showDetail">
			...
			<div class="info">
				...
				<p>
					<i class="title">状态：</i>
					<span class="value">
                        <el-tag type="success" v-if="clerkDetail.disabled==0">正常</el-tag>
                        <el-tag type="danger" v-else-if="clerkDetail.disabled==1">异常</el-tag>
						 <el-tag v-else-if="clerkDetail.deleted==1">已删除</el-tag>
                    </span>
				</p>
		...
		<article class="search">
			<Region :province="fetchParam.provinceSelect" :city="fetchParam.citySelect" :area="fetchParam.areaSelect" :town="fetchParam.townSelect" :village="fetchParam.villageSelect" title="部门" v-on:provinceChange="val => {fetchParam.provinceSelect = val;finallyVal = val}" v-on:cityChange="val => {fetchParam.citySelect = val;finallyVal = val}" v-on:areaChange="val => {fetchParam.areaSelect = val;finallyVal = val}" v-on:townChange="val => {fetchParam.townSelect = val;finallyVal = val}" v-on:villageChange="val => {fetchParam.villageSelect = val;finallyVal = val}" :change="fetchData">
			</Region>
			<section>
				<i>姓名</i>
			...
			<section>
				<i>角色</i>
				<el-select v-model="fetchParam.role_id" @change="fetchData">
		...
		<el-table class="data-table" v-loading="loadingData" :data="tableData" :fit="true" @select="selectRow" @select-all="selectRow" border>
			...
			<el-table-column min-width="150" label="姓名" v-if="data">
				<template scope="scope">
					<span>{{scope.row.name}}</span>
					<i class="wei_icon" v-if="scope.row.wx_id !==''"></i>
				</template>
